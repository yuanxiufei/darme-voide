---
name: multi-shot
trigger-words: [multi-angle, change angle, change shot, multi shot, change angle, new camera angle]
description: |
  Re-generates an existing image from a completely new camera angle. The user drags a 3D sphere control to set rotation/tilt/zoom, and the nano banana model re-renders the original image from that angle. The GUI only handles parameter collection; results are presented in the main site conversation flow.
allowed-tools: hub_open_remote_tool_gui, hub_submit_dag
---

# Multi Shot

Drag a 3D sphere to adjust the camera angle and give an image a new perspective

## Parameters

| ID | Type | Required | Constraints | Description |
|----|------|----------|-------------|-------------|
| image_url | file(image/*) | Yes | — | Original image reference, injected by the main site from the conversation flow; the GUI does not upload or validate formats. Can be a CDN URL (http/https), blob:, data:, or local path — passed through as-is to the backend. |
| view_prompt | string | Yes | — | View angle description prompt (English natural language). Automatically assembled by the GUI from rotation/tilt/zoom parameters when the user clicks "Use Now"; external inject will be overridden. |
...
**Attachment pre-fill (this tool's file parameter: `image_url`)**:
...
|----------|--------|------------|
| `image_url` | `image/*` | First path in the attachment list matching this MIME type |
...
- Should call: `open_remote_tool_gui({ tool_name: "multi-shot", entry: "scripts/index.js", initial_params: { image_url: "/Users/me/face.webp" } })`
...
| inputs | `{ "image_url": "<cdn_url>", "view_prompt": "<View angle description prompt (English natural language). Automatically assembled by the GUI from rotation/tilt/zoom parameters when the user clicks 'Use Now'; external inject will be overridden.>" }` |
