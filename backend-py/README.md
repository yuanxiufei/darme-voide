...
| S4 媒体服务 | image/video/TTS 生成、`compose`(ffmpeg)、宫格、合并、视觉图 | ✅ **主链全通**：`vendor-errors` + text/image/video/TTS+voice-clone 四条链路 + 逐镜路由 + `videos`(6) + `compose`(3) + `merge`(2) + **`grid`(4)**；**像素处理(校色/参考图压缩)** 与 **镜头 QC 打分** 已于 2026-09-15 落地接线（ffmpeg / 规则实现，见下「已知差异」表） |
| S5 **Mastra 替换** | Agent 循环 + 工具调用 + 协议 + **运行时** + 6 组工具(~2900 行) + `agent`(2) | ✅ **完成**：`protocol` + `tool` + **6 组工具** + **`runtime`(运行时：失败分类/退避/模型 fallback/风格注入)** + `agent`(2 端点，**非流式**)；✅ **`DEFAULT_PROMPTS` 已搬**（`services/agent_prompts.py` + 逐字守卫，2026-09-12 决策变更）；✅ `subagent`、`rhythm-phase`（连同 `skills`/`mcp`）**均已迁并接线**；⚠️ **Gemini 函数调用循环未支持**（显式报错） |
...
| ⚠️ **参考音频的绝对 URL 默认指向 5789（Node 的旧端口）** | `to_public_media_url` 沿用原 TS 的默认基址 `http://localhost:5789`（**有意保留，且有测试锁住** —— 改默认值要连带改 `prompt_storyboard_test.py` 的期望）。**必须设 `PUBLIC_BASE_URL=http://localhost:5790`**，否则本地 H3 服务按 5789 拉参考音频会 404；Node 已删，5789 上**没有任何东西在听**（此前的「绞杀期两边都在则无感」已不成立） |
...
| ✅ 媒体生成端点（`characters` 的 `generate-image`/`three-views`/`equip-image`/`expressions`/`batch-generate-images`/`generate-voice-sample`、`scenes` 与 `props` 的 `generate-image`、`storyboards` 的 `generate-tts`/`regenerate-image`/`regenerate-frame`/`set-frame`） | **2026-09-15 校正：全部已迁并注册**（`services/image_generation.py`、TTS、ffmpeg 抽帧）。此处曾记「不注册 → 走反代/501」，而 Node 已删、反代路径不复存在 |
| `DELETE /storyboards/:id` **只清 `storyboard_characters`，不清 `storyboard_props`** | 继承自 Node 的行为（会留下 props 关联孤儿行）。照抄未改，但要知情 —— 若日后要清，**单边改即可**（Node 已删，无需再两边同改） |
| ✅ `POST /characters/:id/generate-prompt` **已迁** | 依赖的 `prompt-utils` 6/6 块完成后即接线（`routers/characters.py`，纯预览、不落库）。此处曾记「需作为独立一次移植」 |
| ✅ 节奏相位 / 时代背景 AI 提炼 / 续写剧本 / 一致性 QC / 宫格 Agent 提示词 —— **均已迁**（2026-09-15） | 对应端点全部注册（**未注册 0 条**，绞杀者收口） |
| ✅ **`PUT /app-settings` 画风白名单缺陷已修**（2026-09-25） | 原缺陷：白名单只列 **6 种**而画风体系有 **10 种** ⇒ `noir` / `ink-wash` / `cyberpunk` / `pixar3d` 被 400 拒绝，**前端设置页那 4 张卡片点了必报错**。Node 删除后，「单边修会让切域那一刻行为静默改变」的约束消失 ⇒ 改为直接引用唯一权威 `services/prompt_utils.ART_STYLE_KEYS`（由 `ART_STYLE_CATALOG` 派生），不再保留第三份副本；`smoke_test.py` 的缺陷锁定用例已翻转为「10 种全通过 + 非法值仍 400」（反套套逻辑） |
