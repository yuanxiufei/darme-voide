...
⭐⭐ **同会话追加（2026-09-26 ✓）：H3 续拍/重拍自检 `engine_h3_edit_test` 上线 ✓ —— 顺手在真链路里
   抓出一条**静默兜底**的产品缺陷 ✗✗**（清单 **+1 套** ✓；⚠️ **全量没跑** ✗ 按规则**不推算**总数 ✗）：
① ⭐⭐ **缺陷（产品侧 ✓ 已修 ✓ 两条证据互证）**：`TorchBackend._dual_carry_pin` 那条「`carry_latents
   ['frames']` 与**视频槽数**对得上」的守卫，原来读 `getattr(plan, "temporal_compression", None)` ✗ ——
   而 `GenerationPlan` 上**根本没有**这个字段 ✓（压缩比是 `GenerationRequest` 的 ✓，plan 只存换算结果
   `latent_frames` ✓）⇒ 恒为 `None` ⇒ `if compression:` 永不成立 ⇒ **在真管线里永不触发** ✓✗
   （拿「读不到」当「没问题」＝本仓最忌的静默兜底 ✓）。⚠️ 第二条互证：它连 `geometry.latent_frames(…)`
   的**调用签名**都是错的 ✗（压缩比是**关键字**参数 ✓）⇒ 真跑过一次早 `TypeError` 了 ✓ ⇒ 确实从没跑过 ✓。
   ⇒ 改法 = 压缩比由调用方**显式**递进来（`init_dual_latents` 手里就有 `request` ✓）、缺了**响亮报错** ✓
   （口径同它上面 `plan.latent_frames` 那道守卫 ✓）；自检 **㊶′** 专钉这条 ✓（原来那套判据抓不到它 ✗）。
② 自检侧修掉 **5 处「断言与事实相反」** ✓（逐条有出处 ✓ **不是**「改测试让红变绿」✗）：
   ⓐ **②**：`snap_grid(200)` 实为 **192** ✓（到 192 差 8 / 到 209 差 9 ⇒ 就近取前者 ✓），原文写 209 ✗
      —— 恰好把「就近不是向上」举成了它的**反例** ⇒ 另补 `snap_grid(201) == 209` ✓；产品 docstring
      里同一处错值一并更正 ✓（`h3_edit.snap_grid` ✓）。
   ⓑ **⑭**：「`end <= start`**要报错」原来打在 `latent_window` 上 ✗ —— 那是**纯映射** ✓，守卫在
      `plan_retake` 上 ✓ ⇒ 改打上层 ✓。
   ⓒ **㉖/㉞ 的 σ**：原文写 `time_shift_sigma(1.0, 12, 3) = 0.25` ✗（实为 **1.0** ✓ —— σ=1 是任意 shift 的
      **不动点** ✓）；且 ㉖ **同时**要求 `sigmaAudio0 == sigmaVideo0` **和** `!=` ✗✗ ⇒ 自相矛盾 ✓
      （σ₀=1 时两条流必然相等 ✓ 是数学 ✓ 不是 bug ✓）⇒ 拆成 **㉖**（σ₀=1 ⇒ 如实钉「两条都 = 1」✓）
      + **㉖′**（σ₀=0.5 ⇒ 音频 **0.2** ✓ 手算 base = 0.5/(12−11×0.5) = 1/13 ⇒ 3·(1/13)/(1+2/13) = 3/15 ✓）✓。
   ⓓ **㉝**：第二段拿「头部之后」比**未折算**的噪声 ✗（隐含 σ₀ = 1 ✓）⇒ 改成 `σ₀·noise` ✓。
   ⓔ **㊴**：尾巴张量写死 **1 通道** ✗ ⇒ 先撞上「不是同一套潜空间」那条（更靠前 ✓）⇒「还长」那条轮不到 ✓✗
      ⇒ 通道/空间照抄本次窗口 ✓。
③ ⚠️ **σ 日程必须只有一份 ✓**（自检侧）：`_dual_sigma0` 取 `plan.sigmas[0]` ✓、采样器吃**显式递进去**
   的那份 ✓（口径同 `pipeline.py` ✓）⇒ 自检原来喂 `[1.0, 0.5, 0.0]` 而 plan 上还是 karras 首值 **14.61** ✗
   ⇒ 钉头按 14.61 铺 ✓、采样器按 1.0 走 ✓✗（㉝ 实跑差 48.65 ✓ 正是这么来的 ✓）⇒ 已对齐 ✓。
④ ⭐⭐ **接线审计又抓出一条"静默腐烂"** ✓（产品侧 ✓ 已补 ✓）：新模块 `h3_edit` **漏登记**进
   ``engine.__all__`` ✗ —— 而全仓**没有**任何测试管这件事（只有 `engine_chain_test` ⑩ 钉它**自己**
   那个名字 ✓）⇒ 是"磁盘上有、清单里没有"的老毛病 ✓。**已补** ✓ 并把它写进下面 55 号守卫 ✓
   （⚠️ 不是"顺手加的"：**先扫源码证实没人管** ✓ 再补 ✓）。
⑤ ⭐ **新增接线守卫 ㊾–56** ✓（静态扫源码 ✓ —— 先例 `engine_chain_test.case_wiring` ✓、
   `cpu_budget_test` ✓）：``chain`` 切尾巴 ✓ → ``segments`` 旁挂（**必须在 `replace` 之后** ✓）→
   ``pipeline`` 递**同一个 request** ✓ → 后端从 `request` 读 `carry_latents` 与压缩比 ✓。
   ⚠️ 为什么非要它：本轮修掉的缺陷**恰恰是"接线看着全在、真路里永不触发"** ✓ ⇒ 光验函数行为不够 ✗。
   ⚠️ 判据只扫**代码行** ✓（注释里引用旧写法是**有意留的** ✓ ⇒ 全文比对会把它误伤成红 ✗）；
   并配**负控 56** ✓：旧写法摆回去 ⇒ 同一判据**当场为假** ✓（证它**不是空守卫** ✗ —— 同款做法见
   盘符守卫那条负控 ✓）。
⑥ **受影响套件逐个实测**（不推算 ✗；⚠️ 下面这些**都是 `__init__.py` 补登记之后**跑的 ✓）：
   `engine_h3_edit_test` **61/61** ✓（新登记 ✓；53 → 61 = 本轮的接线守卫 8 项 ✓）、
   `engine_dual_stream_test` **69/69** ✓、`engine_io_test` **59/59** ✓、`engine_segments_test` **54/54** ✓、
   `engine_chain_test` **45/45** ✓、`engine_refine_test` **23/23** ✓、`engine_pipeline_test` **119/119** ✓、
   `engine_h3_form_test` **83/83** ✓、`engine_core_test` **34/34** ✓、`engine_bridge_test` **73/73** ✓、
   `engine_runtime_test` **31/31** ✓ ⇒ 合计 **651/651** ✓、零红 ✓（⚠️ 本轮**没跑全量** ✗）。
...
TESTS = [
    ...
    ("自研引擎·H3 续拍/重拍（网格窗口与钉长的口径 + 冻结区掩码 + 潜变量接棒："
     "**续拍头部逐位等于上一段尾巴** ✓ / 两条流各自的 σ₀ / "
     "`frames`·压缩比·形状三条守卫都不静默兜底 / **接线静态守卫**：chain 切 → segments 旁挂 → "
     "pipeline 递同一个 request → 后端从 request 读）", "engine_h3_edit_test.py"),
