...
与多种安装方式（runtime）：
  ...
  - git     : git clone 到 backend-py/app/local_services/<key>（如 CosyVoice 2 独立服务）
...
# ⚠️ 三级溯源（本会话踩过两次同类）：本文件在 ``backend-py/app/scripts/`` 下 ⇒ 仓库根要
#    **上跳三级**（scripts → app → backend-py → 仓库根）。跳少了会得到 ``backend-py/app`` 或
#    ``backend-py``，于是 models.json / configs 全找不到，且**不会报错**、只是清单为空 ✗。
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(SCRIPT_DIR)))
#: `app/` 根（``backend-py/app``）—— 「本地服务根」的默认值落在这里（2026-09-15 从仓库根
#: ``local_services/`` 并入 ``app/local_services/``）。它是 `git clone` 下来的独立服务与项目
#: 自带 `h3` 薄封装的落脚点。
#: ⚠️ 同步常量：``app/services/local_model_scan.py`` 与 TS 侧 ``local-model-scan.ts``。
APP_ROOT = os.path.dirname(SCRIPT_DIR)  # = backend-py/app（本脚本在 app/scripts/ 下）
