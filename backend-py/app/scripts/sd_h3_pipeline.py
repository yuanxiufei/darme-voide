...
用法：
    ...
    python backend-py/app/scripts/sd_h3_pipeline.py doctor          # 单步：仅体检
    python backend-py/app/scripts/sd_h3_pipeline.py download        # 单步：仅下载
    python backend-py/app/scripts/sd_h3_pipeline.py build           # 单步：仅编译
    python backend-py/app/scripts/sd_h3_pipeline.py probe [--live]  # 单步：仅判定
    python backend-py/app/scripts/sd_h3_pipeline.py --help
...
# ⚠️ 与 model_manager.py 同理：本文件在 ``backend-py/app/scripts/`` ⇒ 仓库根上跳**三级**
