...
为什么需要
  ...
  得手敲四道命令，容易漏跑 —— 而漏跑的那道恰恰可能是红的那道。

与 ``test_guards.py`` 的分工
  · 本脚本 = **跑**守卫（正向体检：真实资产当前是否合规）。
  · ``test_guards.py`` = **测**守卫（负向实证：守卫还能不能报错）—— 三道守卫都在它里面。
...
STEPS = [
    ...
    # ⚠️ 2026-09-25 新增（实测事故 ✓）：脚本裸跑崩、全量自检却全绿的**静默掩盖** ✗
    ("测试 / CLI 脚本的裸跑编码（GBK 控制台别崩 ✗）", "check_cli_encoding.py", []),
    ("守卫自检（三套守卫的负向用例）", "test_guards.py", []),
]
