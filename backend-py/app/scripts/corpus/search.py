...
⚠️ **同口径的 Agent 工具在 Python 侧**（``app/agent/tools/script_tools.py`` 一带，
...
用法（⚠️ PowerShell 直接传中文参数会乱码 ⇒ 优先用 ``--q-file``）::
    ...
    python backend-py/app/scripts/corpus/search.py --q-file=tmp/q.txt --top=5
    python backend-py/app/scripts/corpus/search.py "rainy neon street" --top=5
    CORPUS_QUERY=雨夜霓虹 python backend-py/app/scripts/corpus/search.py   # 环境变量亦可
...
def main() -> int:
    ...
    if not JSONL.exists():
        print("缺少 data/prompt-corpus/_normalized/prompts.jsonl —— "
              "先跑 python backend-py/app/scripts/corpus/normalize.py", file=sys.stderr)
        return 2
