...
⚠️ 打分口径与 ``backend-py/scripts/corpus/search.py`` **保持一致**（2-gram + 位置加权）：改这里要同步改那边，
...
from ..tool import Tool, json_number, json_string, object_schema
...
_NOT_AVAILABLE_HINT = (
    ...
    "请先执行 python backend-py/scripts/corpus/fetch_raw.py 采集、python backend-py/scripts/corpus/normalize.py 归一化。"
