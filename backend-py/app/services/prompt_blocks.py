"""可复用提示词规范块（移植自 ``backend/src/shared/prompt-blocks.ts``，107 行）。

**用途**：同一套规范要出现在多个位置时（Agent 的 system prompt、工具返回值的 instruction、
skill 文档），**只在本文件定义一次**，各处引用，杜绝副本漂移。

**动因**：格式化剧本规范此前在 3 处各存一份（``DEFAULT_PROMPTS.script_rewriter``、
``script-tools.rewrite_to_screenplay`` 的 instruction、``skills/script_rewriter/SKILL.md``），
三份已出现表述差异（示例、措辞、工具名不一致）⇒ 统一收口到此处。

**约定**（沿用原文件）：
* 命名 ``<领域>_<用途>``；
* 块内只写**跨场景稳定**的规范（格式/铁律/范例），一次性措辞留在调用处；
* 注释里标注「引用方」，便于改动前评估影响面。

⚠️ 这四个块**逐字对齐** TS 源（含示例文本与中文标点）—— SKILL 文档与 Agent 提示词都引用它们，
改字即漂移；``tests/route_parity_test.py`` 的 ``_prompt_blocks_drift`` 会逐字比对。
"""
...
#:
#: 引用方：``DEFAULT_PROMPTS.script_rewriter``、``script_tools.rewrite_to_screenplay``
SCREENPLAY_FORMAT_RULES = """输出硬性要求（必须严格遵守）：
...
IMAGE_PROMPT_TEMPLATE_CHARACTER = """结构骨架（英文，单段）：
...
#:
#: 引用方：``DEFAULT_PROMPTS.extractor``
IMAGE_PROMPT_TEMPLATE_SCENE = """结构骨架（英文，单段，纯背景无人）：
...
IMAGE_PROMPT_TEMPLATE_SHOT = """结构骨架（英文，多行）：
