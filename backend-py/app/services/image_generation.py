...
✅ **校色**（``color_grade.apply_color_grade_to_file``）与**参考图压缩**
（``file_storage.read_image_as_compressed_data_url``）**均已落地**（2026-09-15，ffmpeg 实现，
**刻意不依赖 sharp / Pillow**），分别接在落盘后与参考图归一化两处。与原实现的行为差异
（编码器导致的体积差、亮度近似）见 README「已知差异」表；校色失败仍走「告警 + 保留原图」分支。
