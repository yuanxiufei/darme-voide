...
- ⚠️ **stub 少一个端点 ⇒ 那一支永远验不到** ✗✗（只有 `/object_info/{cls}` ⇒ 加速链永远判「没查」✓✗，
  绿得无意义 ✓）；⭐「是不是克隆路径」要提前知道 ⇒ 写**廉价探针** ✓（查行 + 存在性 ✓ **不读文件** ✗）。⇒ 细节 `TOPICS.md` §接线细则 ✓
- ⭐⭐ **口径「没核到」⇒ 写进报告 + 自检钉住** ✓，**核到了再实现** ✓；⚠️ 别只翻一个文件就下结论 ✗✗
  （掩码/采样这类口径常在**节点文件** ✓ 不在网络文件 ✓）。已核到的超清二采口径 ⇒ `TOPICS.md` §超清二采口径 ✓
