...
## 视频提示词语料
**详见 `TOPICS.md`**（检索管线 8987 条/3 源 ✓、已排除源清单 ✓、落盘三分 ✓）。红线：他人提示词正文**不得搬运进仓库** ✗（只提炼范式 ⇒ 结论落 `docs/` ✓）。
...
- ⚠️⚠️ **CLI 入口必须 `sys.stdout.reconfigure(encoding="utf-8", errors="replace")`** ✗✗（2026-09-25 ✓）：本机是 **GBK** 控制台
  （py3.14 仍按 locale ✗，PEP 686 要 3.15 ✓）⇒ 裸跑**第一条 `print` 就崩** ✓✗；⚠️ **全量自检兜不住** ✗（`run_all.py` 灌了 `PYTHONIOENCODING` ✓）
  ⇒ 守卫 `check_cli_encoding.py` ✓（**老写法 `sys.stdout = io.TextIOWrapper(...)` 不算数** ✗）+ 明细 `TOPICS.md` §CLI 裸跑编码 ✓。
