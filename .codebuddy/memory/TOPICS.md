...
|---|---|---|---|
| `Comic-drama` | 剧本 → 结构化场景/角色资产/对白音频/分镜复核/剪辑时间线的开源漫剧流水线（早期 local-first 原型） | ⚠️ **①TTS 不成立** ✗（2026-09-26 深挖**更正** ⑰ ✓）：`scripts/tts_engines.py` 是**外部服务适配层** ✓（`edge`=`edge_tts` 在线 ✓ / `local`=pyttsx3·SAPI ✓ / 其余 `urlopen` POST 到 `*_TTS_URL` ✓），回退链**只按引擎名分支**（无模型探测 ✓），全仓 **0 处**声学模型/声码器/ONNX/torch 推理 ✗ ⇒ **「自研 TTS」在参考仓里没有现成答案** ✗，只可借**回退链 + `tts_diagnostics()` 字段**这套编排形状 ✓；⭐ **④真价值**（判据可照搬 ✓）：角色 = 颜色直方图×0.6 + ahash×0.4 ≥ **0.6** ✓、道具 0.55/0.45 ✓、风格 0.7 ✓、光照 = `1−(亮度差×0.6+对比差×0.4)` ✓、镜头 = **纯规则罚分**（camera family 突变被情绪/意图变化「合理化」✓）；治理 = **五维** + `policy.mode`(`report`\|`block`) + `deliverable` 门 + `build_continuity_ledger()` 汇总 ✓；`canonical_timeline`（OTIO 式 JSON 契约 ✓ 携带 shot 级 provenance ✓）；**⑤** `workflows/{comfyui_keyframe,asset,faceid}_template.json` = ComfyUI **API 格式** + `__PLACEHOLDER__` 字符串替换 ✓（只有 `faceid` 那个做角色一致性注入 ✓） | MIT |
| `ComfyUI-H3-Multishot` | H3 多镜头**无缝一镜到底**节点包（无可见剪切 / 无色偏 / 不断音） | ⭐ **2026-09-26 第二批取证（本行更正 ⑰ ✓）**：值得 = `h3_retake.py`（**冻结 latent + 窗内 `noise_mask`** 局部重绘 ✓、视频/音频可分开 ✓）+ `h3_extend.py`/`h3_multishot_utils.py` 的 `context_pin`（**钉上一段 latent 尾段 + 音频参考** ✓ ≠ 解码后尾帧 ✓✗）；✗ **不值得**：`h3_keyframes`（绑宿主 VAE/`PackedLayout` ✗）、`h3_lora_stack`（`comfy.sd` 薄包装 ✗）、`h3_gguf_arch`（改别人 `IMG_ARCH_LIST` 全局集合 ✗）；⚠️ **许可分两套** ✗：根 `h3_*.py` = MIT ✓，而 `writer_pack/**`（**含 `libs/ltx_core/*`、`ltx_distillation/*`**）无 LICENSE + README 声明**仅学术/非商用** ✗ ⇒ ⑰ 把它列为「条件注入 / VAE / 量化参照」**作废** ✗；其中唯一**纯 torch**、而我们**全无**的是 `components/guiders.py` 的 **STG/APG** ✓ ⇒ **不可搬，自己按论文写** ✗。`h3_chain_normalize.py` 已抄 ✓（⑲）| MIT（`writer_pack/**` 除外 ✗） |
...
| `ComfyUI-MiniMax-H3-Turbo` | H3 联合音视频、**4 步**采样 + Turbo LoRA | ⭐ **纯 torch 可搬** ✓（2026-09-26 第二批取证 ✓）：`SHIFT_V, SHIFT_A = 12.0, 3.0` ✓ + `_time_shift_sigma/_slope` **闭式纯标量** ✓；双流 = **同一串 sigmas 当视频时钟、音频侧做 shift 映射** ✓；⭐ `_FrugalLoRA.bypass_forward` = **in-place `add_`**（stock 要 3 份临时张量 ⇒ 这里 1 份 ✓ 数值等价 ✓，仅限 Linear ✓）；⚠️ 我们引擎**目前没有任何 LoRA 路径** ✗（`lora` 只在 `accel_chain`/`mappings`/`tiers` 的字面量里 ✓）；⚠️ 宿主耦合 ✗：基类 `comfy.weight_adapter.LoRAAdapter` / `load_lora` / `KSAMPLER` / `ModelSamplingAV`（**native 分支要避免 double-shift** ✓ 这条判据值得抄 ✓）；单文件 554 行 ✓ | Apache-2.0 |
...
| `comfy-org/*`（**⑰ 漏了它** ✗ 2026-09-26 补 ✓） | **Comfy-Org 组织 84 个仓库的整仓镜像**（`ComfyUI/` 本体 + `workflow_templates/` + `docs/` + 各语言 SDK ✓） | ⭐ **`workflow_templates/` = MIT** ✓ ⇒ **7 个 H3 模板** ✓（`video_minimax_h3_{t2v,r2v,i2v,i2v_continuation}.json` ✓ + 3 个 API 版 ✓，落点 `local_services/h3/workflows/` ✓ —— 现只有 `MiniMax_H3_Fast_{T2V,I2V}.json` 两个 ✓ ⇒ **续拍模板是新增能力** ✓）；`site/src/lib/demos/mmh3/config.ts` = 采样器 `res_multistep/euler/dpmpp_2m/ddim/uni_pc` ✓ + 调度器 `simple/normal/karras/beta/sgm_uniform` ✓ + ⭐ **帧数 17k+5** ✓（与 `geometry.H3_FRAME_GRID` **互证** ✓✓）+ 分辨率 8 的倍数 ✓（**事实性常量来源** ✓）；`site/public/demos/mmh3/agent-prompt.md` ✓（提示词规范 ⇒ 对照本仓 H3 提示词层 ✓）；⚠️ **`ComfyUI/` 本体 = GPL-3.0** ✗ ⇒ 其**原生** H3 实现（`comfy/ldm/minimax/{model,vae,audio_vae,controlnet}.py` ✓、`comfy_extras/nodes_minimax_h3.py` ✓、`comfy/text_encoders/minimax.py` ✓、`comfy/latent_formats.py` ✓）**只可读事实、不可搬代码** ✗（可当**第二个来源互证** ✓） | MIT（模板 ✓）/ **GPL-3.0**（`ComfyUI/` ✗） |

⭐ **取证结论（⑰ 前 3 名 ✓，2026-09-26 修正 ✗）**：`ComfyUI-MiniMax-H3-Turbo`（**纯 torch 可搬** ✓ 双流 shift + 省显存 LoRA 加法 ✓）> `ComfyUI-H3-Multishot`（**retake / extend 两个范式** ✓，⚠️ 但 `writer_pack/**` 非商用 ✗）> `Comic-drama`（**④ 治理判据** ✓；⚠️ ①TTS 作废 ✗）；`MiniMax-H3-Codex-Drama`（Skill/契约 ✓）与 `comfy-org` **MIT 模板** ✓ 并列次席。
