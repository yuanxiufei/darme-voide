...
- **引擎本体不 vendor**，但**接口层在项目里**（2026-09-16 补上 ✓）：
  `app/local_services/h3/comfyui_client.py`（`/prompt` `/history` `/view` `/free` `/object_info` `/upload/image`，**零依赖** ✓）
  + `app/local_services/h3/workflow.py`（**UI 图 → API 格式**转换 + 参数注入 —— 参考工作流是 UI 格式、
  而 ComfyUI 的 `/prompt` **只吃 API 格式** ✗，这层转换是「能不能跑」的关键 ✓）。
  契约由 `tests/h3_comfyui_test.py` 守卫（起**真 HTTP stub ComfyUI** ⇒ 不需要 GPU ✓）。
- A5000 跑法：INT8 + offload（FL2VA INT8 ≈ 19.5GB）；第一版**不上 Turbo**，先跑通人物 / 声音 / 嘴型 / 连续性。
- **模型安装统一入口** `backend-py/app/scripts/model_manager.py`（Python 标准库零依赖，清单外置 `configs/models.json`）：

| 命令 | 作用 |
|---|---|
| `python backend-py/app/scripts/model_manager.py list [--category video\|text\|image\|tts] [--missing]` | 列清单与安装状态 |
| `python backend-py/app/scripts/model_manager.py download --required \| --category video \| --all` | 按需批量安装 |
| `python backend-py/app/scripts/model_manager.py install-nodes [--required]` | 装 ComfyUI 自定义节点 |
| `python backend-py/app/scripts/model_manager.py doctor` | 体检（底座/git/ollama/磁盘/模型/节点） |
| `python backend-py/app/scripts/model_manager.py add-model/remove-model` | 增删清单条目（无需改代码） |

  旧入口 `backend-py/app/scripts/h3_install.py` 已降级为兼容 shim（H3 8 模型 + 4 节点），数据源同样指向 `models.json`，新模型请走 `model_manager.py`。
...
- ✅ 本地 H3 服务代码就绪：复用 `minimax` adapter + baseUrl（无需新写 ComfyUI adapter）；模型清单统一外置 `configs/models.json`，通用工具 `backend-py/app/scripts/model_manager.py` 下载/安装/体检；后端 `services/local-model-scan.ts` + `routes/localModels.ts` 扫描本机模型并注册到 `ai_service_configs`（含 H3 `checkpoint_map` 派生）。2026-09-16 补：**ComfyUI 接口层 + UI→API 转换已就位并有自检** ✓（`h3/comfyui_client.py` + `h3/workflow.py`
  + `tests/h3_comfyui_test.py` 39 项 ✓）。2026-09-17 补：**`_run_h3` 已接线** ✓ —— body → 组装/注入
  （提示词 / 时长→帧数 / 画幅 / 权重 / 种子）→ 首尾帧上传（含激活模板里 mute 的尾帧通道并接回主节点 ✓）
  → 提交 → 轮询 → 取片 → `/files` 回 URL → **`/free` 卸载显存** ✓；自检 `tests/h3_stage2_test.py` 20 项 ✓
  （stub ComfyUI，**不需要 GPU / 不下载模型** ✓）。**真机只差**装模型与节点包后跑一轮 ✓
  （`model_manager.py download --category video` + `install-nodes` ✓；缺节点时任务会**点名**是哪个类 ✓）。
