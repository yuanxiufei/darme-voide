...
- ⬜ 本地 H3 服务（复用 `minimax` adapter + baseUrl，无需新写 ComfyUI adapter）。
- ⬜ 参考音频两层语义落地（voice / tts 环节）。
- ⬜ `LOCATION_ID` / `COSTUME_ID` / `STYLE_ID` 三键。
- ⬜ QC（lip_sync / character_consistency / continuity）。
