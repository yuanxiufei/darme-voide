...
> **用途**：维护 `backend-py/app/skills/prompt-style-library` 的词表时，去哪找新词。
>
> **为什么不在 skill 正文里**：skill 的正文会被**注入给 Agent**（每次生成都算进上下文预算），
> 而 Agent **不能上网**——这些站点表对它零价值。放在这里既保留知识，又不让每次生成白背一段。
>
> **合规红线**见 `backend-py/app/skills/prompt-style-library/SKILL.md` §7 与本文末节。
...
| **PromptMart 谱码** | 短剧 / 漫剧专用 | **只取标签与范式**（已归纳进 `backend-py/app/skills/video-prompt-library` 的 6.x 节）：打斗、变身、分镜板类镜头指令（标签如 `#真人实拍` `#一镜到底` `#冲击波`）；正文多需付费解锁，**不要付费取词** |
...
以下两个是**外部技能库**（`backend-py/app/skills/production-tools/`），不默认注入，
...
- 只提炼**范式与词表**，**不得把他人提示词正文整段搬进仓库**（合规红线，与 `backend-py/app/skills/video-prompt-library` 的 6.9 节一致）。
