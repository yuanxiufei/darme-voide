...
export function createGridPromptTools(episodeId: number, dramaId: number) {
  ...
  const generateCharacterPrompt = createTool({
    ...
    execute: async ({ character_id }) => {
      ...
      const prompt = `${base}, ${VISUAL_STYLE_MASTER}`
  ...
  const generateGridPrompt = createTool({
    ...
    execute: async ({ shots, rows, cols, mode, reference_legend }) => {
      ...
      if (mode === 'multi_ref') {
        const sb = shots[0]
        const gridPrompt = `${rows}x${cols} grid layout, exactly ${totalCells} visible panels, ${VISUAL_STYLE_MASTER}, ${legendPrefix}${sb.description}, all cells with identical lighting and color palette, no merged panels, no missing panels`
        const cellPrompts = Array.from({ length: totalCells }, (_, i) => ({
          shot_number: sb.shot_number,
          frame_type: 'reference',
          prompt: `格${i + 1}：${reference_legend ? `参考${reference_legend}，` : ''}${sb.description}, ${VISUAL_STYLE_MASTER}, consistent with other cells in the ${rows}x${cols} grid`,
        }))
      ...
      if (mode === 'first_last') {
        ...
        const gridPrompt = `${rows}x${cols} grid layout, exactly ${totalCells} visible panels, ${VISUAL_STYLE_MASTER}, ${legendPrefix}${shots.map(s => s.description).join(' | ')}, no merged panels, no missing panels`
        return { grid_prompt: gridPrompt, cell_prompts: cellPrompts }
      }
